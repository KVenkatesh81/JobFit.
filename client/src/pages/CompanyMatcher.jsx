...
export default function CompanyMatcher() {
  ...
  const match = async () => {
    ...
    setResult(null);
    try {
      const { data } = await api.post('/ai/company-match', {
        resumeId: selectedId,
        preferences,
      });
  ...
  return (
    <div className="min-h-screen bg-[#0f0f13]">
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <p className="text-slate-400 mb-8">Find companies that fit your profile + discover your best career paths</p>
            ...
            {activeTab === 'companies' && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {(result.companies || []).map((c, i) => (
                  <div key={i} className="bg-[#17171f] border border-white/5 rounded-2xl p-5 hover:border-indigo-500/30 transition-all">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h3 className="font-display text-white font-bold text-lg">{c.name}</h3>
                        <div className="flex items-center gap-2 mt-1">
                          <span className={`text-xs px-2 py-0.5 rounded-full border ${typeColor[c.type] || typeColor.MNC}`}>
                            {c.type}
                          </span>
                          <span className={`text-xs ${difficultyColor[c.difficulty]}`}>
                            {c.difficulty} to get in
                          </span>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className={`font-display text-2xl font-bold ${c.match_score >= 80 ? 'text-green-400' : c.match_score >= 60 ? 'text-yellow-400' : 'text-red-400'}`}>
                          {c.match_score}%
                        </div>
                    ...
                    <p className="text-slate-400 text-xs mb-3">{c.why_match}</p>
                    <p className="text-slate-500 text-xs mb-3 italic">Known for: {c.known_for}</p>
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          ...
                          <span className={`text-xs font-medium ${demandColor[path.demand]}`}>
                            {path.demand} demand
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="flex gap-2 items-center flex-wrap">
                      {(path.steps || []).map((step, j) => (
                        <div key={j} className="flex items-center gap-2">
                          <span className="text-sm text-slate-300 bg-[#0f0f13] border border-white/5 px-3 py-1.5 rounded-lg">
                            {step}
                          </span>
                    ...
                    <ul className="space-y-2">
                      {result.immediate_actions.map((a, i) => (
                        <li key={i} className="flex items-start gap-3 text-sm text-slate-300">
                          <span className="text-indigo-400 font-bold font-display">{i + 1}.</span>
                          {a}
                        </li>
                      ))}
                    </ul>
